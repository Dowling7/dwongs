...
def getData(fname="", procName="Events"):
    ...
    kn_events = ak.zip({"Electrons":ak.zip({
                        ...
                        "st23":ak.zip({
                                      ...
                                      "st3_x":   kn_dict_ak1["st23tracklet_x_st3"],
                                      "st3_y":   kn_dict_ak1["st23tracklet_y_st3"],
                                      "st3_z":   kn_dict_ak1["st23tracklet_z_st3"],
                                      "st3_px":   kn_dict_ak1["st23tracklet_px_st3"],
                                      "st3_py":   kn_dict_ak1["st23tracklet_py_st3"],
                                      "st3_pz":   kn_dict_ak1["st23tracklet_pz_st3"],
...
def Clustering_multi(file):
    ...
    for i in range(len(eng_eve)):
        ...
        labels.append(label)
        labels_decrease.append(label_clus_eng(label, eng_eve[i]))
        num_clus.append(max(label)+1)

    return labels, labels_decrease, x_eve, y_eve, eng_eve, num_clus
#--------------------------------------------------------------------------------------------------------------------------------------------
def clus_coord(file):
    (labels, labels_decrease, x_tup, y_tup, eng_tup, num_clus)=Clustering_multi(file)
    ...
    for i in range(len(labels)):#loop through evts
        ...
        for item in labels_decrease[i]:
            ...
            xvals = x_tup[i][indices]
            yvals = y_tup[i][indices]
            energies = eng_tup[i][indices]
            evt_coord.append([xvals, yvals, energies])#cluster coord in []
        coord.append(evt_coord)
    return coord
#--------------------------------------------------------------------------------------------------------------------------------------------
def CAL_st23(file):#unfinished
#extrapolate to CAL, use EMCal centers as base, match track center at radius 10, closest, only use once.
    dq_events = getData(file)
    st3_x= dq_events["st23"].st3_x.tolist()#st3 may catch multiple tracklets or no tracklet
    st3_y= dq_events["st23"].st3_y.tolist()
    st3_z= dq_events["st23"].st3_z.tolist()
    st3_px= dq_events["st23"].st3_px.tolist()
    st3_py= dq_events["st23"].st3_py.tolist()
    st3_pz= dq_events["st23"].st3_pz.tolist()
    x_Cal=[]
    y_Cal=[]
    for i in range(len(st3_x)):
        x_evt=[]
        y_evt=[]
        for j in range(len(st3_x[i])):
            x_evt.append(st3_x[i][j]+(st3_px[i][j]/st3_pz[i][j])*(1930-st3_z[i][j]))
            y_evt.append(st3_y[i][j]+(st3_py[i][j]/st3_pz[i][j])*(1930-st3_z[i][j]))
        x_Cal.append(x_evt)
        y_Cal.append(y_evt)
    return x_Cal, y_Cal
