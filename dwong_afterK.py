...
def track_bytuple(file):
    ...
    pz = dq_events["track"].pz
    return track_x, track_y, pz

#--------------------------------------------------------------------------------------------------------------------------------------------
def matchup(file):
    output1 = gen_center(file)
    output2 = valid_track(file)
    wew_x = output1[0]
    wew_y = output1[1]
    track_x = np.hstack(output2[0])
    track_y = np.hstack(output2[1])
    diff_x = []
    diff_y = []
    for i in range(len(track_x)):
        if (track_x[i]==-9999 and track_y[i]==-9999):
            diff_x.append(-9999)
            diff_y.append(-9999)
        else:
    ...
    return np.nan_to_num(np.hstack(diff_x), nan=-9999), np.nan_to_num(np.hstack(diff_y), nan=-9999)
#--------------------------------------------------------------------------------------------------------------------------------------------
...
def valid_track(file):
    ...
    #fill empty with -9999, check multi points
    indecies=[]
    for i in range(len(p)):
        if p[i]==[]:
            p[i].append(-9999)
            track_x[i].append(-9999)
            track_y[i].append(-9999)
        elif len(p[i])!=1:
            indecies.append(i)
